...
require(auksRuak)
require(sfext) # https://github.com/elipousson/sfext
...
create_colored_bm = function(DT, lat = 'lat', lon = 'lon', buffer = 1000, sc_dist, squared = FALSE,
                             projection = paste0('+proj=laea +lat_0=90 +lon_0=-156.653428 +x_0=0 +y_0=0',
                                                 ' +datum=WGS84 +units=m +no_defs +ellps=WGS84 +towgs84=0,0,0 ')){
  ...
  if(nrow(DT) > 0) {
    ...
    bm =
      ggplot() +
      geom_sf(data = land, fill = '#faf5ef') + #f6eee2  #f8f2e9
      geom_sf(data = lakes[lakes$fclass == 'wetland', ], fill = '#faf5ef', alpha = 0.6, colour = '#faf5ef') +
      geom_sf(data = lakes[lakes$fclass == 'water', ], fill = '#f3fafd', colour = 'grey80') + # #D7E7FF
